...
class DetailViewModelTest {
    ...
    @Test
    fun `should notify uiState with Success from UiState when get character categories returns success`() = runTest {
        // Arrange
    ...
    fun `should notify uiState with Success from UiState when get character categories returns only events`() =
        runTest {
            ...
            whenever(getCharacterCategoriesUseCase(any()))
                .thenReturn(
                    flowOf(
                        ResultStatus.Success(emptyList<Comics>() to events)
                    )
                )
    ...
    fun `should notify uiState with Empty from UiState when get character categories returns an empty result list`() =
        runTest {
            ...
            whenever(getCharacterCategoriesUseCase(any()))
                .thenReturn(
                    flowOf(
                        ResultStatus.Success(emptyList<Comics>() to emptyList())
                    )
                )
    ...
    @Test
    fun `should notify uiState with Error from UiState when get character categories returns an exception`() = runTest {
        // Arrange
        whenever(getCharacterCategoriesUseCase(any()))
            .thenReturn(
                flowOf(
    ...
    fun `should notify favorite_uiState with filled favorite icon when check favorite returns true`() =
        runTest {
            ...
            whenever(checkFavoriteUseCase.invoke(any()))
                .thenReturn(
                    flowOf(
                        ResultStatus.Success(data = true)
                    )
                )
    ...
    fun `should notify favorite_uiState with not filled favorite icon when check favorite returns false`() =
        runTest {
            ...
            whenever(checkFavoriteUseCase.invoke(any()))
                .thenReturn(
                    flowOf(
                        ResultStatus.Success(data = false)
                    )
                )
